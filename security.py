...
This module centralizes the rules the app uses to stay safe when dealing with
user-supplied files and on-disk vector stores:
...
Keeping these rules in one place makes them easy to audit and reuse.
"""

import os
import re

# Root directory for FAISS vector stores. Everything we load/save lives under
# here; we never read or write indexes from arbitrary user-supplied paths.
VECTOR_STORES_DIR = os.path.realpath(os.path.join(os.getcwd(), "vector_stores"))

# Only PDF is enabled for now. Add new extensions here (lowercase, with dot)
# when the app gains support for them.
ALLOWED_EXTENSIONS = (".pdf",)

# Reject uploads larger than this to avoid memory/disk exhaustion.
MAX_FILE_SIZE_MB = 25
